...
export const FlowerAnimation = ({ objectId }) => {
  ...
  useFrame((state, delta) => {
    ...
    const elapsedSinceStart = elapsedTime % 10 // Reset every 10 seconds for continuous blooming
    
    // Stem growth (0-0.5 seconds)
    const stemPhase = Math.min(1, elapsedSinceStart / 0.5)
    ...
    const flowerPhase = elapsedSinceStart > 0.4 ? Math.min(1, (elapsedSinceStart - 0.4) / 0.6) : 0
    ...
    if (elapsedSinceStart > 0.5) {
      flowerGroupRef.current.rotation.z = Math.sin(elapsedTime * 2) * 0.05
    }
    
    // Set animation as complete after 10 seconds, but actually keep running
    // This prevents any state changes that might cause it to disappear
    if (elapsedSinceStart > 9.5) {
      setAnimationComplete(true)
      
      // Reset after a delay to create continuous animation
      setTimeout(() => {
